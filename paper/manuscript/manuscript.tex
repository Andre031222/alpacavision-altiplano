...
%% Force the target journal metadata (the vendored class expects the bare option
%% `animals`; we set the names explicitly so headers/footers read "Animals").
\makeatletter
...
%% Uniform, slightly airier table row spacing (consistent across all tables)
...
%% ORCID: real, confirmed iDs for all four authors.
\Author{Richar Andre Vilca-Solorzano$^{1,*}$\,\href{https://orcid.org/0009-0003-2385-5263}{\orcidicon},
        Dina Maribel Yana-Yucra$^{1}$\,\href{https://orcid.org/0009-0003-6218-2735}{\orcidicon},
        Cristian Daniel Ccopa-Acero$^{1}$\,\href{https://orcid.org/0009-0005-7176-5849}{\orcidicon}
        and Leonid Alem\'an-Gonzales$^{1,*}$\,\href{https://orcid.org/0000-0002-4072-6370}{\orcidicon}}

\AuthorNames{Vilca-Solorzano, R.A.; Yana-Yucra, D.M.; Ccopa-Acero, C.D.; Alem\'an-Gonzales, L.}
...
\address{$^{1}$~Semillero de Investigaci\'on ``John J.\ Hopfield -- IIICCD'',
         ...
         75521963@est.unap.edu.pe (R.A.V.-S.);
         maribelbel201314@gmail.com (D.M.Y.-Y.);
         danielccopa76@gmail.com (C.D.C.-A.);
         laleman@unap.edu.pe (L.A.-G.)}

\corres{Correspondence: 75521963@est.unap.edu.pe (R.A.V.-S.);
        laleman@unap.edu.pe (L.A.-G.)}

\abstract{%
The Peruvian Altiplano holds 87\% of the world's alpacas (\textit{Vicugna pacos}), yet
detection of morphological anomalies relies on subjective visual inspection by scarce
veterinarians, and no public computer-vision dataset or automated system exists for the
species. We present \textbf{AlpacaVision AI}, comprising: (i)~the first open, curated and
deduplicated dataset of 2,051 unique labelled alpaca images from nine public sources
(1,037 MD5 duplicates removed), CLAHE-preprocessed for high-UV Andean conditions;
(ii)~a compact YOLOv11n body detector reaching mAP@0.5~=~0.860 (P~=~0.913, R~=~0.731) on a
leakage-free 308-image test set; (iii)~a feasibility study showing that vision-language
auto-labelling of ocular anomalies is insufficient for fine-grained classification: under a
leakage-free protocol an EfficientNet-B2 classifier attains AUC-ROC~=~0.506
(95\%~CI~[0.366, 0.645]), indistinguishable from chance, which we trace to unreliable
labels and a physical resolution ceiling requiring expert veterinary ground truth and
close-up imagery; (iv)~a data-integrity methodology (MD5 deduplication, group-aware
splitting, and label-conflict resolution) that corrected systematic leakage inflating
earlier metrics; and (v)~a deployed web application with automated Spanish veterinary
reporting and ONNX export. The dataset (CC~BY~4.0) and code (MIT) are released to support
reproducible precision livestock farming and improved welfare in high-Andean alpaca
communities.
}
...
families~\cite{morante2009,fowler2010}.
...
\begin{figure}[H]
  ...
  \caption{Dataset composition (leakage-free).
           \textbf{(A)}~MD5 deduplication of the detector corpus: 3,088 raw files, 1,037
           exact duplicates removed, and 2,051 unique images retained.
           \textbf{(B)}~Leakage-free detector split of the 2,051 unique images
           (train 1,435 / validation 308 / test 308).
           \textbf{(C)}~Real, non-augmented ocular-crop inventory for the classifier
           feasibility study (371 normal / 91 anomaly), partitioned group-aware.}
  \label{fig:dataset_stats}
\end{figure}

\subsubsection{Source Aggregation}

We consolidated alpaca images from nine public Roboflow Universe alpaca-detection
projects, all providing YOLO-format bounding-box annotations for the single class
\texttt{alpaca}. The exact project identifiers and the per-image source are listed in the
released dataset record and accompanying data card (DOI:~10.5281/zenodo.21134001), so the
provenance is fully reproducible. One further candidate project was excluded after quality
screening (mean bounding-box area ratio $< 0.01$, indicating non-informative annotations).
...
\begin{table}[h]
\caption{AlpacaVision AI detector dataset composition and leakage-free splits (after MD5
         deduplication of 3,088 raw files to 2,051 unique images).}
\label{tab:dataset}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Split} & \textbf{Images} & \textbf{Fraction} & \textbf{Annotation} \\
\midrule
Train      & 1,435 & 70\% & YOLO bbox \\
Validation & 308   & 15\% & YOLO bbox \\
Test       & 308   & 15\% & YOLO bbox \\
\midrule
\textbf{Total} & \textbf{2,051} & \textbf{100\%} & YOLO bbox \\
\bottomrule
\addlinespace[2pt]
\multicolumn{4}{@{}l}{\footnotesize Stratified split ($\mathrm{seed}=42$) performed on
unique source images, so no augmentation}\\
\multicolumn{4}{@{}l}{\footnotesize or duplicate of a test image can appear in the training
set.}\\
\end{tabular}
\end{table}
...
using open-set region proposals rather than the single-class body detector.
Two anatomical regions of clinical priority were targeted: ocular regions (eyes) and
distal limbs (legs). Candidate regions were localised with Grounding
DINO~\cite{liu2023grounding} open-set detection, which produced a five-class pseudo-label
scheme (body, head, eye, front leg, rear leg); we retained the ocular
(\texttt{alpaca\_eye}) and distal-limb (\texttt{alpaca\_leg}) proposals and cropped each
with a fixed-ratio padding of 10\% around the predicted box.
Ocular-anomaly labels were then generated using zero-shot vision-language inference
(Groq \texttt{meta-llama/llama-4-scout-17b-16e-instruct}, a Groq-hosted multimodal
(vision-language) model;\footnote{Model card:
\url{https://console.groq.com/docs/model/meta-llama/llama-4-scout-17b-16e-instruct}.})
\emph{without} veterinary expert validation ---
...
the ocular crop set: (i)~\emph{augmentation leakage} --- the 873 ``anomaly'' crops originally
counted were augmentation copies of far fewer source images, with augmentations of the same
image leaking across train/test; and (ii)~\emph{label conflicts} --- of the unique
``anomaly''-labelled images, 75 were the \emph{same file} (identical MD5) as a ``normal''
image, an artefact of an extraction step that copied rather than moved crops.
After removing augmentation leakage and resolving these conflicts by MD5 matching, only
\textbf{91 unique anomalous eye images} remained; Table~\ref{tab:crops} reports the honest
unique-image inventory.
For the classifier, the 462 usable eye crops (371 normal / 91 anomaly) were partitioned
with the same 70\%/15\%/15\% train/validation/test scheme as the detector, using a
group-aware split that keeps every augmentation of a source image on a single side and
draws the validation and test folds only from unaugmented originals; the frozen test set
therefore comprises 70 unique images ($\approx$15\% of 462: 56 normal / 14 anomaly).
...
\begin{table}[h]
\caption{Anatomical crop inventory for the classifier feasibility study, after MD5
         deduplication and label-conflict resolution. Counts are unique source images.}
\label{tab:crops}
\small
\begin{tabular}{@{}lccclc@{}}
\toprule
\textbf{Region} & \textbf{Normal} & \textbf{Anomaly} & \textbf{Total} &
\textbf{Labelling method} & \textbf{Usable} \\
\midrule
Eyes & 371 & 91 & 462 & Auto (Groq Vision, unvalidated) & Yes \\
Legs & 443 & 3  & 446 & Auto (Groq Vision, unvalidated) & No\textsuperscript{a} \\
\bottomrule
\addlinespace[2pt]
\multicolumn{6}{@{}l}{\footnotesize \textsuperscript{a}Only 3 anomalous leg crops --- too
few to train a classifier; leg anomaly detection is deferred to future work.}\\
\multicolumn{6}{@{}l}{\footnotesize Extracted eye crops have a median smaller side of
$\approx$31~px (Section~\ref{sec:discussion}), a physical resolution ceiling}\\
\multicolumn{6}{@{}l}{\footnotesize inherited from low-resolution source imagery.}\\
\end{tabular}
\end{table}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{fig6_transfer_learning.png}
...
Crucially, in light of the feasibility study (Section~\ref{sec:res:classifier}), the
production backend deliberately \emph{bypasses} the chance-level EfficientNet-B2 classifier:
ocular assessment in the deployed application is delegated to the Groq vision-language model,
while the compact YOLOv11n detector provides the reliable animal-localisation step. No
model that performs at chance is used for diagnosis in the deployed system.
...
\caption{Stage-1 alpaca body detector on the leakage-free held-out test set
         ...
         larger YOLOv11s capacity ablation. Precision and Recall are reported at the
         default confidence threshold; best value per row in \textbf{bold}. All metrics
         were reproduced from the released checkpoints (\texttt{best\_clean.pt},
         \texttt{best\_clean\_s.pt}).}
\label{tab:detector}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
...
\midrule
\multicolumn{3}{@{}l}{\textit{Detection accuracy}} \\
\quad mAP@0.5        & 0.860 & \textbf{0.863} \\
\quad mAP@0.5:0.95   & 0.693 & \textbf{0.711} \\
\quad Precision      & \textbf{0.913} & 0.903 \\
\quad Recall         & 0.731 & \textbf{0.766} \\
\addlinespace
\multicolumn{3}{@{}l}{\textit{Model efficiency}} \\
\quad Parameters           & \textbf{2.58M} & 9.43M \\
\quad GFLOPs               & \textbf{6.3}   & 21.5 \\
\quad Model size (PyTorch) & \textbf{5.3 MB} & 18.3 MB \\
\quad Model size (ONNX)    & 11.0 MB & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{fig3_detector.png}
...
\begin{figure}[H]
  ...
  \caption{YOLOv11n detection examples on held-out test images, shown for two test
           batches (rows). Left column: ground-truth bounding boxes; right column:
           YOLOv11n predictions with confidence scores.
...
vision-language auto-labels alone and requires expert veterinary ground truth. Detailed ROC,
...
\caption{EfficientNet-B2 ocular anomaly classifier --- leakage-free test-set performance
         ($n=70$: 14 anomaly / 56 normal). Percentile bootstrap 95\% CI ($B = 1000$,
         seed~=~42). An AUC-ROC CI straddling 0.5 indicates chance-level discrimination.}
...
Accuracy         & 0.586 & --- \\
\midrule
$n_\text{test}$ (anomaly / normal) & \multicolumn{2}{c}{70 (14 / 56)} \\
...
approaching the range reported for larger architectures in related livestock tasks such as
sheep detection (mAP@0.5 $\approx$ 0.88--0.95,~\cite{kong2022sheep}) and cattle monitoring
systems~\cite{neethirajan2020}, despite the greater visual heterogeneity of our
multi-source dataset. Notably, a larger YOLOv11s model reached only
...
for fine-grained ocular pathology in camelids, vision-language pseudo-labels do not encode a
...
tempted to bootstrap medical-grade classifiers from vision-language-model annotations.
...
\begin{enumerate}
  \item \textbf{Auto-labelling is insufficient for fine-grained anomaly classification:}
        The ocular labels were produced by zero-shot vision-language-model inference without
  ...
  \item \textbf{Geographic and phenotypic scope:}
        The Roboflow training images are globally sourced; the iNaturalist subset
        covers Peru but was not restricted to the altiplano biome. Moreover, the corpus
        may under-represent the marked phenotypic variation among alpaca breeds --- notably
        the dense, crimped fleece of the \textit{Huacaya} versus the long, lustrous locks of
        the \textit{Suri} --- and across coat colours; such differences in body outline and
        surface texture could reduce detector recall under large-scale field deployment.
        Domain adaptation across breeds, coat types, and high-altitude Andean imaging
...
Zenodo at \href{https://doi.org/10.5281/zenodo.21134001}{doi.org/10.5281/zenodo.21134001}
(DOI:~10.5281/zenodo.21134001) under Creative Commons Attribution 4.0 (CC~BY~4.0).
